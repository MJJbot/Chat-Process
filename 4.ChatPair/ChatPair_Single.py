...
class ChatAnalysis:
    ...
    def save_chat_similar_pairs(self, pair_num=100,\
        ...
        filename_high = f"ChatPairs_high_{self.data_size}.csv.txt"
        filename_mid = f"ChatPairs_mid_{self.data_size}.csv.txt"
        filename_low = f"ChatPairs_low_{self.data_size}.csv.txt"
        
        output_file_high = codecs.open(filename_high, 'a', encoding='utf-8')
        output_file_mid = codecs.open(filename_mid, 'a', encoding='utf-8')
        output_file_low = codecs.open(filename_low, 'a', encoding='utf-8')
        
        chat_range = self.get_chat_chunk_range()
        
        chunk_info = f"\nChunk {self.chunk_num:4d} (#{chat_range[0]} ~ #{chat_range[1] - 1})\n"
        output_file_high.write(chunk_info)
        output_file_low.write(chunk_info)
        output_file_mid.write(chunk_info)
        
        candidate = np.where(self.sim_matrix >= low_lower_th)
        ...
        for idx in range(pair_num):
            ...
            #print(f"{self.chunk_num}: {idx+1} / {pair_num}")
        ...
        if self.verbose:
            ...
            print(f"Chat mid pairs are saved to {filename_mid}")
...
def main(start, end):
    ...
    for idx in range(start, end):
        ...
        chat_analysis.save_chat_similar_pairs(pair_num=150, \
                                              option={"high":{"upper_threshold": 1.00, "lower_threshold": 0.985, "ratio": 0.333},\
